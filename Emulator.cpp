...
void Emulator::initialize() {
    ...
    while(config >> key) {
        // A robust way to read space-separated key-value pairs
        char nextChar = config.peek();
        while(isspace(nextChar) && nextChar != '\n') {
            config.get();
            nextChar = config.peek();
        }
        std::getline(config, value);
        // Trim potential leading/trailing whitespace from value
    ...
    if(scheduler_type == "rr"){
        ...
        scheduler = new Scheduler(num_cpu, SchedulingMode::fcfs);
...
// Method to display the process status inside the process screen.
...
// Method to handle 'screen -r' from the main menu.
...
void Emulator::handleMainCommand(const std::string& input) {
    if (input == "initialize") {
        ...
        if (checkInitialized())
            scheduler->writeScreenListToFile("csopesy-log.txt");
    } else if (input == "clear") {
        clearScreen();
    } else if (input == "exit") {
